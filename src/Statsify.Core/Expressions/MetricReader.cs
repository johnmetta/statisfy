...
namespace Statsify.Core.Expressions
{
    ...
    {
        public static List<Metric> ReadMetrics(Environment environment, ISet<string> metricNames, DateTime from, DateTime until)
        {
            //
            // Do a single pass through queued metric datapoints
            var queuedDatapoints = 
                environment.QueuedMetricDatapoints.
                    Where(md => metricNames.Contains(md.Name) && from <= md.Datapoint.Timestamp && md.Datapoint.Timestamp <= until).
                    GroupBy(md => md.Name.ToLowerInvariant()).
                    ToDictionary(g => g.Key, g => g.Select(md => md.Datapoint).ToList());

            var metrics = new List<Metric>(metricNames.Count);
